...
import { Search, Calendar, Users, Star, MapPin, Wifi, Car, Coffee, Waves, Bed, Bath, Building2, Filter, SlidersHorizontal } from 'lucide-react';
...
const HotelsPage: React.FC = () => {
    ...
    const fetchFeaturedHotels = async () => {
      ...
      try {
        ...
        if (result.success && result.hotels) {
          ...
          setFeaturedHotels(getMockHotels());
        }
      } catch (error) {
  ...
  const handleSearch = () => {
    ...
    // Navigate to hotel search results with search parameters
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-4 xs:gap-6 sm:gap-8">
            {hotelTypes.map((type, index) => (
              <div
                ...
                onClick={() => {
                  const searchParams = new URLSearchParams();
                  searchParams.set('propertyType', type.name.toLowerCase().slice(0, -1)); // Remove 's' from end
                  navigate(`/hotel-search?${searchParams.toString()}`);
                }}
              ...
              >
                ...
                <div className="text-sm xs:text-base text-blue-600 font-semibold">{type.count} hotels</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Loading featured hotels...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12">
