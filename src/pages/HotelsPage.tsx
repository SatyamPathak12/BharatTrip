...
import { Search, Calendar, Users, Star, MapPin, Wifi, Car, Coffee, Waves, Bed, Bath, Building2, Filter, SlidersHorizontal, Plus, Minus } from 'lucide-react';
    ...
    const fetchAllHotels = async () => {
      ...
      try {
        ...
        if (result.success && result.hotels) {
         const transformedHotels = result.hotels.map((hotel: any) => ({
            id: hotel.id,
            name: hotel.name,
...
console.log('Featured hotels:', transformedHotels.filter(h => h.featured));
  ...
  const handleSearch = () => {
    if (!searchData.destination.trim()) { alert('Please enter a destination'); return; }
    if (!searchData.checkIn || !searchData.checkOut) { alert('Please select check-in and check-out dates'); return; }
  ...
  const handleLoadMore = () => {
    ...
    const nextHotels = allHotels.slice(0, currentLength + 4);
    setDisplayedHotels(nextHotels);
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Hero Section */}
      <section 
        className="relative min-h-[70vh] bg-cover bg-center bg-no-repeat pt-16"
        style={{
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white px-3 sm:px-4 max-w-4xl w-full">
            ...
            <p className="text-base xs:text-lg sm:text-xl md:text-2xl mb-4 sm:mb-6 md:mb-8 text-gray-200 px-1 sm:px-2">
              Discover amazing hotels, resorts, and homestays across India
            </p>
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-4 xs:gap-6 sm:gap-8">
            {hotelTypes.map((type, index) => (
              <div
                ...
                onClick={() => {
                  const searchParams = new URLSearchParams();
                  searchParams.set('propertyType', type.filterValue);
                  navigate(`/hotel-search?${searchParams.toString()}`);
                }}
              ...
              >
                ...
                <div className="text-sm xs:text-base text-blue-600 font-semibold">
                  {typeof type.count === 'number' && type.count > 0 ? `${type.count} hotels` : type.count}
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-8 xs:py-12 sm:py-16 bg-white">
        <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-6 xs:mb-8 sm:mb-12">
            ...
            <p className="text-sm xs:text-base sm:text-lg text-gray-600 px-2">
              Browse our collection of {allHotels.length} verified properties
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
                <h3 className="text-lg font-medium text-red-800 mb-2">Unable to Load Hotels</h3>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm font-semibold text-gray-900">{hotel.rating.toFixed(1)}</span>
                          <span className="text-xs text-gray-500 ml-1">(Excellent)</span>
                        </div>
                        <div className="text-right">
                          <div className="text-xl font-bold text-gray-900">₹{hotel.price.toLocaleString()}</div>
              ...
              <div className="text-center mt-12">
                {displayedHotels.length < allHotels.length ? (
                  <button
                    ...
                    className="bg-blue-600 text-white px-8 py-3 rounded-xl hover:bg-blue-700 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105"
                  >
